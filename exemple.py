...
for j in range(8):
    ...
    temperature_factor = worksheet.cell(j, 10).value
    grouping_factor = worksheet.cell(j, 11).value
    soil_resistivity_factor = worksheet.cell(j, 12).value
...
sizing_current = (rated_current*overload_factor)/(temperature_factor*grouping_factor*soil_resistivity_factor)
